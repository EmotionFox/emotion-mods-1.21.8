...
public class FadedBlockItem extends BlockItem {
    public FadedBlockItem(Block block, Properties properties) {
        super(block, properties.component(ComponentRegistry.PHASE.get(), new PhaseRecord(4)).component(DataComponents.LORE, new ItemLore(List.of(Component.translatable("item.emomodore.phase.tooltip").append(": " + 4).withStyle(ChatFormatting.DARK_AQUA)))));
    }

    @Override
    protected boolean placeBlock(BlockPlaceContext context, BlockState state) {
        int phase = context.getItemInHand().get(ComponentRegistry.PHASE).value();
